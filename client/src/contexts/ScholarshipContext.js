...
const createScholarship = useCallback(async (scholarshipData) => {
  try {
    // Remove empty or undefined fields so backend won't reject
    const cleanedData = JSON.parse(JSON.stringify(scholarshipData));

    const response = await axios.post(`${API_BASE}/api/scholarships`, cleanedData, {
      headers: {
        Authorization: `Bearer ${localStorage.getItem("token")}`, // if your backend uses auth
      },
    });

    console.log("Create scholarship response:", response.data);

    const newScholarship = response.data.scholarship || response.data;

    setScholarships((prev) => (Array.isArray(prev) ? [...prev, newScholarship] : [newScholarship]));
    ...
    return {
      success: false,
      message: error.response?.data?.message || error.message || "Failed to create scholarship",
    };
  }
}, []);
