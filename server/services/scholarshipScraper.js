...
class ScholarshipScraper {
  ...
  maxBodyLength: 50 * 1024 * 1024
      ...
      const openToAllPatterns = [
        ...
        /applicable to all .{1,100}(programmes|programs|courses|fields|disciplines)/i,
      ...
      const hasOpenToAll = openToAllPatterns.some(
        (pattern) => pattern.test(pageText) || pattern.test(globalText)
      );
      if (hasOpenToAll) {
        ...
        $('h1, h2, h3, h4, h5, h6, strong, b, div, p').each((i, el) => {
          ...
          const isLooseTag = $(el).is('div, p');
          const looksLikeLabel = headingText.endsWith(':') || headingText.length <= 60;
          
          // Only check if text is short enough to be a heading (not entire page content)
          if (headingText.length > 200) return;
          
          if (possibleSectionHeadings.some(k => headingText.includes(k)) && (!isLooseTag || looksLikeLabel)) {
      ...
      const sanitizeEligibleCourses = (list) => {
        if (!Array.isArray(list) || list.length === 0) return list;
        if (list[0] === "All Fields") return list;
        return list
          .map(normalizeCourse)
          .filter((s) => s && s.length >= 2 && s.length <= 80)
          .filter((s) => !looksLikeCss(s))
          .filter((s) => !looksLikeGrade(s))
          .filter((s) => !looksLikeNav(s))
          .filter((s) => nonLetterRatio(s) < 0.5)
          .filter((s) => !stopwords.some((w) => s.toLowerCase().includes(w)))
          .map((s) => s.replace(/^[-•·\s]+/, ''));
      };
      eligibleCourses = sanitizeEligibleCourses(eligibleCourses);
      ...
      if (shouldUseAI) {
        try {
          ...
          if (aiCourses && aiCourses.length > 0) {
            eligibleCourses = sanitizeEligibleCourses(aiCourses);
          }
        } catch (aiError) {
        }
