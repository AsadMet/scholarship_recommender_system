...
class EmailService {
  constructor() {
    ...
    this.smtpPort = process.env.SMTP_PORT ? Number(process.env.SMTP_PORT) : undefined;
    this.smtpUser = process.env.SMTP_USER;
    this.smtpPass = process.env.SMTP_PASS;
    this.smtpSecure = String(process.env.SMTP_SECURE || '').toLowerCase() === 'true';
    this.smtpFrom = process.env.SMTP_FROM || this.senderEmail;
  ...
  initializeService() {
    ...
    } else if (this.apiKey && this.senderEmail) {
  ...
  isSmtpConfigured() {
    return Boolean(this.smtpHost && this.smtpPort && this.smtpUser && this.smtpPass && this.smtpFrom);
  }

  async sendViaSmtp(email, otpCode) {
    ...
    return { success: true, message: 'OTP sent successfully (SMTP)' };
  }
  ...
  async sendOTPEmail(email, otpCode) {
    try {
      ...
      if (this.isSmtpConfigured()) {
        return await this.sendViaSmtp(email, otpCode);
      }
      ...
      // Fallback: try SMTP if Brevo fails and SMTP is configured
      if (this.isSmtpConfigured()) {
        try {
          return await this.sendViaSmtp(email, otpCode);
        } catch (smtpError) {
          console.error('SMTP sending error:', smtpError.message || smtpError);
        }
      }

      // Fallback to demo mode if all sending methods fail
